...
\hspace{0.3cm}
	...
	the supersymmetric CP($N-1$) two-dimensional model with the $\mc{Z}_N$ twisted masses $m_l$
	($l=0,1, ..., N-1$).
	A related issue we address is that of the curves of marginal stability (CMS). Previous analyses were incomplete.
	...
	they become circles, filling out a band on the plane of a single mass parameter of the model at hand. Inside CMS, 
	$N-1$ towers collapse into $N$ stable states.
\vspace{2cm}
...
\setcounter{equation}{0}
	...
	perspectives: viewed from the bulk and 
	on the world sheet. 
	
	Superalgebra in the CP($N-1$) model is centrally extended, with the central charge containing a topological term and
	$(N-1)$ Noether charges. The former includes the canonic contribution and an anomalous one, 
	which is especially obvious in the limit of the vanishing twisted masses   (e.g. \cite{Shifman:2006bs}). The contributions of the 
	 ...
	 BPS states on the both sides of CMS (as well as the CMS itself)
	 in the CP(1) model  was constructed in  \cite{Shifman:2006bs}. For $N>2$ and $\mc{Z}_N$-symmetric twisted masses
	 the issue of the BPS spectrum and CMS separating the strong and weak-coupling domains was addressed
	 ...
	 explicit  analytic determination of the BPS spectra and CMS, even if possible, 
	   ...
	   defining all twisted masses. On the physical side the advantage of the $\mc{Z}_N$-symmetric
	   choice is also obvious: with this choice the $\mc{Z}_N$ symmetry of the massless model, a remnant of
	   the axial $R$ symmetry, is not broken by the twisted masses. Physics of the transition from weak to strong coupling becomes 
	   transparent.
	...
	These are the states that become massless in the Argyres--Douglas points \cite{AD},
	in  which vacua collide. In our case, the $\mc{Z}_N$-symmetric masses, all vacua collide simultaneously.
	The vanishing mass requirement at this point, in conjunction with small mass formula,
	enable us to resolve  the ambiguity in the VY superpotential and determine
	the masses of the stable BPS states both outside and inside CMS in  its entirety.
	We arrive at the conclusion  that the spectrum found in \cite{Dor} and consisting
	of a single BPS tower (for a given pair of vacua between which the kinks under consideration interpolate)
	...
	That's the tower described in \cite{Dor}. All others are not resolved:
	in the quasiclassical limit of large $ | m_l | $ and large U(1) quantum numbers,
	all $ N-1 $ towers fuse with each other.
	
	With the BPS spectrum in hands we pass to the second stage: 
	determination of  the curves of the marginal stability  in the same CP($N-1$) model
	with the $\mc{Z}_N$-symmetric twisted masses.
	For each of the BPS towers there must be a curve on which the relevant states decay.
	Altogether, we deal here with $N-1$ curves of the marginal stability.
	...
	$m_0$, inside which only the strong coupling states are stable.
	...
	with radii in the interval \beq
	 1 \,\leq\, |m|/\Lambda \,\leq\, e^2\,, \qquad (e= 2.71828...)
	 \eeq
	  in units of the 
	low-energy scale $ \Lambda $.
	In the limit of very large $ N $ the    curves of the marginal stability  completely fill  this interval, forming
	a round band.
	...
	spectrum both at strong and weak coupling basing on the VY superpotential and the conditions outlined above.
	Construction of $N-1$ curves of the marginal stability is the subject of Sec. \ref{curves}. Section \ref{conclu} presents a
...
in the strong-coupling limit $ e_0 \to \infty$. Here $e_0$ is the gauge coupling while
	$ m_i $ are the complex twisted mass parameters which can be introduced \cite{twistedm} through a
	background vector field in four dimensions. Moreover, $2\beta$ can be viewed as a coupling of the
	corresponding sigma model (sometimes people use $r\equiv 2\beta$.)
	...
	$ \sigma $. Thus, one can always impose the condition
	\beq
	\sum_{l=0}^{N-1}\, m_l = 0\,.
	\label{mcondi}
	\eeq
	...
	subgroup 
	of the anomalous U(1)$_\text{R}$ symmetry.  To this end they must lie 
...
\eeq
In passing from (\ref{fullcpn}) to (\ref{Wfull})
we rescaled $ \sigma $,
\beq
	\sigma ~~\to~~ \frac{\sigma}{\sqrt 2}\,.
	\label{resca}
\eeq
Here $ \tau $ is the complexified coupling,
\beq
	\tau ~~=~~ i r ~+~ \frac{\theta}{2\pi}\,, \qquad\qquad\qquad   \text{with~~~} r ~~\equiv~~ 2\beta\,.
\eeq
	The ultraviolet cut-off scale $ \mu $ can be traded for the dynamical scale $ \Lambda $,\footnote{Without loss
	of generality one can always assume $\Lambda$ to be real and positive. We will stick to this convention. A possible phase of $\Lambda$ is absorbed in $m_0$.}
\beq
...
The CP$(N-1)$ model is asymptotically free.
	...
	Now, Witten's formula \cite{W93} for the position of vacua is
	\beq
	\prod_{l=0}^{N-1} \, \left( \sigma -m_l\right) =\Lambda^N\,.
	\label{Wf}
	\eeq
	This is the equation for the critical points of $\W_\text{eff}(\sigma)$. With the set of masses (\ref{mcirc})
	Eq. (\ref{Wf}) implies that the
	vacua of this theory lie at
\beq
\label{sigvac}
...
\eeq
In the future, for
	determination of the BPS spectrum, we will need the values of the superpotential
...
In deriving expression (\ref{Wvac}) we used the Witten relation (\ref{Wf}). With our normalization, the
	general formula for the mass of an {\em elementary} BPS state reduces to the difference of the 
...
which pair of neighbors we pick up. This is due to the  $\mc{Z}_N$ symmetry of the model.
	...
	and focus on the masses of the kinks interpolating between these two,
\beq
\label{mbpsmain}
	\mbps ~~=~~ \big|\, \W_\text{eff} ( \sigma_1 ) ~~-~~ \W_\text{eff} ( \sigma_0 ) \,\big| \,.
\eeq
	(In bulky expressions, we will  omit the absolute value sign, keeping it in mind.  Certainly, $ \mbps $
...
Veneziano--Yankielowicz potential, being a multi-branch function,
	is too ambiguous.
...
The question mark in (\ref{quesm}) reminds us that
	...
	is an arbitrary constant integer vector, $\vec{N}=(n_0,...,n_{N-1})$ with integer $n_i$ and
$\vec{m}=(m_0,...,m_{N-1})$. Now we rewrite \eqref{mbpsmain} as,
\beq
\label{mspec}
...
\end{align}
The branch of $U_0$ is fixed as follows: if $A$ is a real positive number, (i) $\sqrt[N] {A}$ is a real positive number; (ii)
$\ln A$ is a real number. Here $ m_j $ is meant to be a function of $ m_0 $ as well, via \eqref{mcirc}. 
...
\end{itemize}
	...
	We claim that function $ U_0(m_0) $ is single-valued in a domin of the complex plane
...
\setcounter{equation}{0}
	...
	If all masses $ m_j $ sit on the circle the corresponding
	vacua will also be forced to sit on the circle, see
	\eqref{sigvac}.
...
\beq
	\sigma_p ~~=~~ 0\,.
\eeq
	This occurs whenever $m_0^N = - \Lambda^N$, or 
\beq
...
\setcounter{equation}{0}
	...
	We first collect the results known and trustworthy about the CP(1) model \cite{Shifman:2006bs},
...
\eeq
	These are the two states in the strong coupling regime. 
...
\eeq
	...
	In this illustrative example of CP(1), we stayed one sheet of the logarithm. 

	We need to mention, that for CP(1) (and only for CP(1)) our results is not incompatible with those
...
\end{itemize}
	...
	Having formulated the criteria for the spectrum selection    from the 
	variety of branches of the logarithms of the 
Veneziano--Yankielowicz superpotential,
	we can now proceed to higher $ N $.
	Of these, CP(2) is the first  nontrivial example.
...
\subsection{Domain of \boldmath{$ m_0 $}}
	...
	using the formula \eqref{mspec}  (i) in a neighborhood of the
Argyres--Douglas point;
	(ii) in the neighborhood of $ m_0 ~=~ 0 $, and (iii) in the region of large $ |m_0| $, 
	...
	We choose the sector between theñå two 
Argyres--Douglas points
\beq
\label{mdomain}
	m_0^\text{AD} ~~=~~ \Lambda \, e^{i \pi / N}
...
\end{figure}
	...
	Although, as we argued, we do not need that for our purposel, we will partly be able to address the latter aspect.
...
\eeq
	...
	similarly to how we did this when we discussed Eq.~\eqref{ncp1}.
...
\subsection{Spectrum in CP(\boldmath{$N-1$)}}
	...
	One other confirmation of this will be given further, when we will   discuss  the curves
	of the marginal stability in Section~\ref{curves}.
	
	Our discussion of the CP($N-1$) theory will be in vein with  generalization of our
...
\eeq
	...
	As it can be easily seen, in terms of the vector $ \vec{N} $,  this describes the kinks 
...
%                                                                                %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Curves of the Marginal Stability}
\label{curves}
\setcounter{equation}{0}
...
\subsection{The decay curve in CP(1)}
	...
	There is just one curve of the marginal stability in CP(1) and it is the primary one.
...
\epsfxsize=7.5cm
\epsfbox{ccp1.eps}
\caption{The curve of the marginal stability in CP(1) theory ($ m_0^2 $ plane), Ref. \cite{Shifman:2006bs}.}
\label{ccp1}
\end{center}
\end{figure}
...
The subscript $c$ stands for compressed.
	Figure~\ref{ccp2} shows two curves in the rescaled $ m_0^3 $ plane.
\begin{figure}
\begin{center}
\epsfxsize=7.5cm
\epsfbox{ccp2.eps}
...
\eeq
	...
	Figures~\ref{ccp5}-\ref{ccp9}, as an illustration, show the curves for the CP(5), CP(6) and CP(9).
\begin{figure}
\begin{center}
\epsfxsize=7.5cm
\epsfbox{ccp4.eps}
...
\epsfxsize=7.5cm
\epsfbox{ccp5.eps}
\caption{\small The curves in the CP(5) theory (compressed $ m_c $ plane). Outer radius is $ 6.39\, \Lambda $.} 
\label{ccp5}
\end{center}
\end{figure}
\begin{figure}
...
\end{figure}
	...
	Let us briefly review the mainfeatures of these plots. 
...
\setcounter{equation}{0}
	...
	the 
Argyres--Douglas points; quasiclassical spectrum contains a tower of states with
	integer $ U(1) $ charges. The
	knowledge of the exact superpotential is not sufficient as it is  ambiguous.
	However, if one fixes this ambiguity at least in some region of the parameter space,
	then the superpotential must still describe the states  in the strong
	coupling domain.
...
in \cite{SYmon,HT2}. 
The confined 't Hooft--Polyakov monopoles of the four-dimensional theory are represented by junctions of two different non-Abelian strings. The effective theory on the world sheet of the
non-Abelian string is two-dimensional CP$(N-1)$ model \cite{HT1,ABEKY,SYmon,HT2}. Confined monopoles of the bulk theory are seen in the world-sheet theory as kinks of the CP$(N-1)$ model. This ensures the  coincidence of the BPS spectra in both theories, see \cite{SYmon,HT2} for more details.
...
Veneziano--Yankielowicz superpotential.
	...
	contours that correspond to the stable BPS states. In particular, our result \eqref{cpntowers}
for the weak-coupling spectrum of the CP$(N-1)$ model suggests that the dyon electric charges in the bulk theory,
besides  towers built on  roots of the gauge group, contain also contributions determined by its weights,
cf. \cite{FrHoll}.
	The question of the correspondence between the spectra of the BPS states and 
	the decay curves of dyons in  two theories calls for a special investigation.
	
	\section*{Acknowledgments}
	\addcontentsline{toc}{section}{Acknowledgments}
 The work of MS was supported in part by DOE
grant DE-FG02-94ER408. 
The work of P.B. is supported by the DOE grant DE-FG02-94ER40823.
...
%\bibitem{Veneziano:1982ah}
%  G.~Veneziano, S.~Yankielowicz,
%  %``An Effective Lagrangian for the Pure N=1 Supersymmetric Yang-Mills Theory,''
%  Phys.\ Lett.\  {\bf B113}, 231 (1982).
